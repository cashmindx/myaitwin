...
export default function AvatarPreview() {
  ...
  async function handleUpload() {
    ...
    const uploadRes = await fetch("/.netlify/functions/upload-image", {
      ...
      body: JSON.stringify({ fileBase64: base64, publicId: "myaitwin-avatar" })
    });
  ...
  return (
    <div className="max-w-lg mx-auto text-center p-6 bg-black rounded-xl text-white shadow-xl">
      <h2 className="text-2xl font-bold mb-4">🧠 myAITwin: Avatar Preview</h2>
      ...
      >
        {isLoading ? "Creating Twin..." : "Preview Avatar"}
      </button>

      {cloudinaryUrl && (
        <div className="mt-6">
          <h4 className="text-lg mb-2">🖼️ Original Image</h4>
          <img src={cloudinaryUrl} alt="myAITwin" className="rounded-md shadow-md" />
        </div>
      )}

      {videoUrl && (
        <div className="mt-6">
          <h4 className="text-lg mb-2">🎥 Lip-Sync Video</h4>
